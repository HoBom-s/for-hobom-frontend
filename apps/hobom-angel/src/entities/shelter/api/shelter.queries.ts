...
export const shelterQueries = {
  ...
  verification: (shelterId: string) =>
    queryOptions({
      queryKey: [...shelterQueries.all(), shelterId, "verification"] as const,
      queryFn: ({ signal }) => getShelterVerification(shelterId, signal),
    }),
