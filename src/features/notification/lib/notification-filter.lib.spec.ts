...
describe("FILTER_PREDICATES", () => {
  it("all 필터는 항상 true를 반환한다", () => {
    expect(FILTER_PREDICATES.all(makeNotification({ isRead: false }))).toBe(
      true,
    );
    expect(FILTER_PREDICATES.all(makeNotification({ isRead: true }))).toBe(
      true,
    );
  });

  it("unread 필터는 isRead=false일 때 true를 반환한다", () => {
    expect(FILTER_PREDICATES.unread(makeNotification({ isRead: false }))).toBe(
      true,
    );
  });

  it("unread 필터는 isRead=true일 때 false를 반환한다", () => {
    expect(FILTER_PREDICATES.unread(makeNotification({ isRead: true }))).toBe(
      false,
    );
  });

  it("read 필터는 isRead=true일 때 true를 반환한다", () => {
    expect(FILTER_PREDICATES.read(makeNotification({ isRead: true }))).toBe(
      true,
    );
  });

  it("read 필터는 isRead=false일 때 false를 반환한다", () => {
    expect(FILTER_PREDICATES.read(makeNotification({ isRead: false }))).toBe(
      false,
    );
  });
});
